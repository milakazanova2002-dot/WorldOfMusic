...
class AssignmentDetailView(LoginRequiredMixin, DetailView):
    ...
    def dispatch(self, request, *args, **kwargs):
        user = request.user
        assignment = get_object_or_404(TeachingAssignment, pk=kwargs["pk"])

        is_the_teacher = hasattr(user, "teacher_profile") and assignment.teacher_id == user.teacher_profile.id
    ...
    def get_context_data(self, **kwargs):
        ...
        context["lessons"] = Lesson.objects.filter(assignment=assignment).order_by("-date")
        context["performances"] = Performance.objects.filter(assignment=assignment).order_by("-created_at")
